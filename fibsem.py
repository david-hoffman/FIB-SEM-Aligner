...
def _read_header(fobj):
    ...
    if fibsem_header.FileMagicNum != 3555587570:
        raise RuntimeError(
            "FileMagicNum should be 3555587570 but is {}".format(fibsem_header.FileMagicNum)
        )
    ...
    header_dtype.update(
        ["XResolution", "YResolution"], [">u4", ">u4"], [100, 104]  # X Resolution  # Y Resolution
    )
    ...
    if fibsem_header.FileVersion == 8:
        header_dtype.update(
            ["BeamDump2I", "MillingI"],  # Beam dump 2 current (nA)  # Milling current (nA)
            [">f4", ">f4"],
            [882, 886],
        )
